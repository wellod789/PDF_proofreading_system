...
class PDFCorrectorGUI:
    ...
    def create_menu(self):
        ...
        settings_menu.add_command(label="プロンプト編集...", command=self.open_prompt_editor)
        settings_menu.add_separator()
        settings_menu.add_command(label="プロンプトをデフォルトに戻す", command=self.reset_prompts_to_default)
    ...
    def open_prompt_editor(self):
        ...
        # 各プロンプトのテキストエリア
        labels = [
            # 処理順に並べる: テキスト → 画像配置（レイアウト） → 画像分析（視覚） → 統合
            ("テキスト校正プロンプト (text_check)", "text_check"),
            ("画像配置チェック（レイアウト） (image_layout_check)", "image_layout_check"),
            ("画像分析プロンプト（視覚） (image_visual_check)", "image_visual_check"),
            ("統合プロンプト (integration)", "integration"),
        ]

        self.prompt_text_widgets = {}

        for idx, (title, key) in enumerate(labels):
            ttk.Label(frame, text=title, style='Header.TLabel').grid(row=idx*2, column=0, sticky=tk.W, pady=(0, 5))
            text_widget = scrolledtext.ScrolledText(frame, height=6, width=90)
            text_widget.grid(row=idx*2 + 1, column=0, sticky=(tk.W, tk.E), pady=(0, 10))
            text_widget.insert(1.0, prompt_manager.get_prompt(key))
            self.prompt_text_widgets[key] = text_widget

        # ボタン行
        button_frame = ttk.Frame(frame)
        button_frame.grid(row=len(labels)*2, column=0, pady=(10, 0), sticky=tk.E)
        ttk.Button(button_frame, text="保存", command=self.save_prompts).grid(row=0, column=0, padx=(0, 10))
        ttk.Button(button_frame, text="閉じる", command=editor.destroy).grid(row=0, column=1)
    ...
    def save_prompts(self):
        ...
        try:
            errors = []
            for key, widget in self.prompt_text_widgets.items():
                value = widget.get(1.0, tk.END).strip()
                result = prompt_manager.validate_template(key, value)
                if result.get("parse_error"):
                    errors.append(f"{key}: テンプレート構文エラー（波括弧の対応など）")
                    continue
                if result.get("missing"):
                    errors.append(f"{key}: 必須変数が不足: {', '.join(sorted(result['missing']))}")
                if result.get("unexpected"):
                    errors.append(f"{key}: 未知の変数を含む: {', '.join(sorted(result['unexpected']))}")
            if errors:
                messagebox.showerror("エラー", "プロンプトに問題があります:\n\n" + "\n".join(errors))
                return

            # バリデーションOKの場合のみ保存
            for key, widget in self.prompt_text_widgets.items():
                value = widget.get(1.0, tk.END).strip()
                prompt_manager.set_prompt(key, value)
            prompt_manager.save()
            messagebox.showinfo("保存", "プロンプトを保存しました。")
        except Exception as e:
            messagebox.showerror("エラー", f"保存に失敗しました: {str(e)}")

    def reset_prompts_to_default(self):
        """プロンプトをデフォルトに戻す"""
        if messagebox.askyesno("確認", "全てのプロンプトをデフォルトに戻しますか？"):
            prompt_manager.reset_to_defaults()
            messagebox.showinfo("完了", "デフォルトに戻しました。開いている編集画面は再度開いてください。")
    ...
    def run_correction(self):
        ...
        try:
            ...
            if self.ai_enabled:
                self.update_progress("テキスト・レイアウト・画像分析を並列実行中...")
                with ThreadPoolExecutor(max_workers=3) as executor:
                    text_future = executor.submit(self.run_text_analysis_only)
                    layout_future = executor.submit(self.run_layout_analysis_only)
                    image_future = executor.submit(self.run_image_analysis)

                    text_corrections = text_future.result()
                    layout_corrections = layout_future.result()
                    image_corrections = image_future.result()

                # AIでテキスト・レイアウト・画像の結果を統合
                self.update_progress("AIで結果を統合中...")
                self.corrections = self.integrate_analysis_results(text_corrections, image_corrections, layout_corrections)
            else:
                # AI機能が無効の場合はテキスト分析のみ
                self.update_progress("テキスト分析を実行中...")
...
"""
    ...
    def run_layout_analysis_only(self):
        """レイアウトチェックのみを実行（並列処理用）"""
        try:
            self.root.after(0, lambda: self.update_progress("レイアウトチェックを実行中..."))
            layout_corrections = self.corrector.run_layout_analysis_only(
                self.selected_file,
                progress_callback=lambda msg: self.root.after(0, lambda: self.update_progress(f"レイアウト: {msg}"))
            )
            return layout_corrections
        except Exception as e:
            self.root.after(0, lambda: self.update_progress(f"レイアウト分析エラー: {str(e)}"))
            return []
    ...
    def integrate_analysis_results(self, text_corrections, image_corrections, layout_corrections):
        """AIでテキスト分析・画像分析（視覚）・レイアウト結果を統合"""
        try:
            # ページごとにグループ化
            page_groups = {}
            ...
            for correction in text_corrections:
                ...
                if page not in page_groups:
                    page_groups[page] = {'text': [], 'image': [], 'layout': []}
            ...
            for correction in layout_corrections:
                ...
                if page not in page_groups:
                    page_groups[page] = {'text': [], 'image': [], 'layout': []}
                page_groups[page]['layout'].append(correction)
            ...
            for page_num in sorted(page_groups.keys()):
                ...
                layout_results = page_groups[page_num]['layout']
                
                # AIで統合
                integrated_result = self.integrate_page_results_with_ai(page_num, text_results, image_results, layout_results)
    ...
    def integrate_page_results_with_ai(self, page_num, text_results, image_results, layout_results):
        """AIでページのテキスト・画像（視覚）・レイアウト結果を統合"""
        try:
            # テキスト分析結果をまとめる
            text_summary = ""
            ...
            for result in layout_results:
                layout_summary += f"レイアウト: {result['correction']}\n\n"
            
            template = prompt_manager.get_prompt("integration")
            prompt = template.format(page_num=page_num, text_summary=text_summary, image_summary=image_summary, layout_summary=layout_summary)
